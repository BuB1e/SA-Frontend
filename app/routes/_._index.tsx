...
export default function Index() {
  ...
  async function fetchUser() {
    ...
    const userTemp : UserSession = JSON.parse(sessionStorage.getItem("user") || "{}");
    const currentUser = data.find((user) => user.username === userTemp.name);
    console.log(userTemp);
    setUser(data);
    setRole(currentUser?.priority === 0 ? "Supplier" : "Factory");
    setUsername(currentUser?.username || "");
    sessionStorage.setItem("role", currentUser?.priority === 0 ? "Supplier" : "Factory");
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-white px-10 py-10 space-y-10 w-svw h-auto overflow-auto">
      <div className="flex flex-col justify-center items-center w-full h-full border-4 border-black rounded-xl p-20 space-y-12">
        ...
        <h1 className="font-bold text-black text-4xl underline">
          ข้อมูลของคุณ
        </h1>
